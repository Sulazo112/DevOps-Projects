...
4. Let's navigate to the search bar on our AWS management console

a) Enter "VPC." Upon locating the relevant result, proceed to click on it, directing you to the Virtual Private Cloud (VPC) page.

5. Please navigate to the "Create VPC" option and click on it.

![VPC dashboard](./img/VPC%20dashboard.png)
...
![Create subnet top](./img/Create%20subnet%20top.png)

![Create subnet bottom](./img/Create%20subnet%20bottom.png)

So this EC2 instance hosts my E-commerce website.

![EC2 instances running](./img/EC2%20instances%20running.png)
...
![Security outbound](./img/Security%20outbound.png)
...
![Instances public IP](./img/Instances%20public%20IP.png)

If you enter "http:// 51.21.192.90" into our Chrome browser, and hit enter, you'll notice that the page doesn't load; it keeps attempting to connect. And finally it’ll show this page. After some time, you'll likely see a page indicating that the site can't be reached.

![Unreachable web address](./img/Unreachable%20web%20address.png)
...
![Create security inbound](./img/create%20security%20inbound%20rule.png)

f) Now, click on Create security group.

Now you will see the rule have been created.

![Security group successfully created](./img/Security%20group%20successfully%20created.png)
...
![Change security group](./img/Change%20security%20group.png)
...
![Change security group 1](./img/Change%20security%20group%201.png)
...
![Security group added](./img/Security%20group%20added.png)

Now it is being attached successfully,

a) If you again copy the public IP address

![SG changed successfully](./img/SG%20changed%20successfully.png)

b) And write http:// 51.21.192.90 in Chrome, We’ll be able to see the data of our website.

![Public IP address](./img/Public%20IP%20address%20.png)
...
![HTTP and SSH protocols access the instance.](./img/HTTP%20and%20SSH%20protocols%20access%20the%20instance..png)

The outbound rule permits all traffic to exit the instance.

![Outbound rule](./img/HTTP%20and%20SSH%20protocols%20access%20the%20instance..png)

Through this rule, we're able to access the website.
![Web Piexe](./img/Public%20IP%20address%20.png)
...
![Edit SG Outbound rule](./img/Edit%20SG%20Outbound%20rule.png)

c) Click on “Delete.”

d) Click on “Save rules.”

![Delete Outbound rule](./img/Delete%20Outbound%20rule.png)

Now that we've removed the outbound rule, let's take a look at how it appears in the configuration.

![No security group rule found](./img/No%20security%20group%20rule%20found.png)

After making this change, let's test whether we can still access the website.

![Pixie Ecommerce](./img/Piexie%20Ecommerce.png)
...
![Inbound tab](./img/Inbound%20tab.png)

C) Click on delete

d) Click on “Save rule.

![edit inbound rule](./img/Edit%20inbound%20rule.png)

Currently, let's have a look at how our inbound and outbound rules are configured.

![No inbound security group rule found](./img/No%20inbound%20security%20group%20rule%20found.png)

![No outbound security group rule found](./img/No%20Outbound%20security%20groupo%20rule%20found.png)
...
![Page not loading](./img/Page%20not%20loading.png)
...
![Http traffic in the outbound rule](./img/Http%20traffic%20in%20the%20outbound%20rule.png)
...
![Http edit outbound rule](./img/Http%20edit%20outbound%20rule.png)

![Outbound SG successfully modified](./img/Outbound%20SG%20successfully%20modified.png)

Now, let’s see if we can access the website

![No web page](./img/No%20Webpage.png)
...
![Cloudshell curl http:](./img/Cloudshell%20curl%20http.png)
...
![Create network ACL](./img/Create%20network%20ACL.png)
...
![NACL successfully created](./img/NACL%20successfully%20created.png)
...
![Similarly outbound rule](./img/Similarly%20outbound%20rule.png)
...
![inbound rule edit](./img/inbound%20rule%20edit.png)
...
![All traffic edit inboundb rule](./img/All%20traffic%20edit%20inboundb%20rule.png)

Currently, this NACL is not associated with any of the subnets in the VPC.

![No subnet associated with](./img/No%20subnet%20associated%20with.png)
...
![Public subnet assosciation](./img/Public%20subnet%20association.png)
...
![Successfully updated subnet association](./img/Successfully%20updated%20subnet%20association.png)

As soon as you have attached this NACL to your public subnet, and then you try to access the website again by typing the URL http:51.21.192.90, you will notice that you are unable to see the website Note: There may be change in Public IP because the Instance was stopped and start in another day.

![Web cant be reached](./img/Web%20cant%20be%20reached.png)
...
![Allowing and Denying traffic](./img/Allowing%20and%20Denying%20traffic.png)

![Outbound denying traffic](./img/Outbound%20denying%20traffic.png)

![Net Setup](./img/Net%20Setup.png)
...
![Duplicate outbound rule edit](./img/Duplicate%20outbound%20rule%20edit.png)

We have successfully created the rules

![Outbound rule successfully updated](./img/Outbound%20rule%20successfully%20updated.png)

Upon revisiting the website, you should now be able to access it without any issues.

![Website accessed](./img/Piexie%20Ecommerce.png)
...
![Configuring SG inbound](./img/Configuring%20SG%20inbound.png)

![SG inbound rule successfully configured](./img/SG%20inbound%20rule%20successfully%20configured.png)

![SG Outbound rule](./img/SG%20Outbound%20rule.png)

NACL

Let’s remove NACL so by default it will denied all traffic.

![NACL inbound rule removed](./img/NACL%20inbound%20rule%20removed.png)

As seen the NACL has been removed and all traficc deny to the subnet.

![As seen inbound rule removed](./img/As%20seen%20inbound%20rule%20removed.png)

Additionally, the outbound rule will be removed, defaulting to deny all traffic by default.

![Removed Outbound rule](./img/Removed%20Outbound%20rule.png)

All trafiic deny after removing the NACL

![All traffic denied](./img/All%20traffic%20denied.png)

Now, let’s try to access the website

![UnreachableWebAddress](./img/Unreachable%20web%20address.png)
